...
$required = ['id', 'category', 'title', 'description', 'user_id'];
...
try {
    ...
    $stmt = $pdo->prepare("UPDATE info_hub_items SET category=?, title=?, description=?, contact_info=?, action_link=?, action_text=? WHERE id=?");
    $stmt->execute([$category, $title, $description, $contact_info, $action_link, $action_text, $id]);
